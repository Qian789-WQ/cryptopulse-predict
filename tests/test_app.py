...
class RouteTests(unittest.TestCase):
    ...
    def test_frontend_guards_websocket_symbol_switch(self):
        ...
        self.assertIn("function getPortfolioCapacity", html)
        self.assertIn("function renderActiveOrderDetails", html)
        self.assertIn("function orderNetPnlAtPrice", html)
        self.assertIn("function orderReservedNotional", html)
        self.assertIn("系统会自动缩仓，不再弹窗中断", html)
        self.assertIn("系统采用总保证金", html)
        self.assertIn("价格先离开再接近时才提示", html)
        self.assertNotIn("盈利加仓二仓候选", html)
